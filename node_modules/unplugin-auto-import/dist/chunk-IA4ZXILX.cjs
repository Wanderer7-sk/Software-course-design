...
function createContext(options = {}, root = process.cwd()) {
  ...
  const cachePath = isCache === false ? false : _path.resolve.call(void 0, root, typeof isCache === "string" ? "string" : "node_modules/.cache/unplugin-auto-import.json");
...
`;
  ...
  async function generateCache() {
    ...
    try {
      ...
      await Promise.allSettled(Object.keys(cacheData).map(async (filePath) => {
        try {
          await _fs.promises.access(_path.posix.resolve(root, filePath));
        } catch (e) {
  ...
  async function updateCacheImports(id, importList) {
    ...
    await unimport.modifyDynamicImports(async (imports) => {
      const cacheData = await getCacheData(cachePath);
      if (id && importList) {
        const filePath = _path.posix.normalize(_path.relative.call(void 0, root, id));
